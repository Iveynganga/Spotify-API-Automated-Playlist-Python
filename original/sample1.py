...
client_id = CLIENT_ID = "65e4d985b21645ab9eb6cff676e17655"
client_secret = CLIENT_SECRET = "2f41919cbfb248539a8db2330d364457"
spotify_token = "2f41919cbfb248539a8db2330d364457"
...
#iterate over each YT video title in yt_song_list
        for i in self.yt_song_list:
            search_query = "https://api.spotify.com/v1/search?query=track%3A{}&type=track&offset=0&limit=1".format(i) #Spotify search query URL for the current YT video title
            ...
            self.uri_list.append(response["tracks"]["items"][0]["uri"])  #extract the URI of the first track from the response and append it to uri_list
...
#send a POST request to the Spotify API to create the new playlist
        ...
        new_playlist_id = response["id"]    #get the ID of the new playlist from the response
...
#convert the dictionary containing URIs to JSON format
        ...
        # Print the response for adding songs to the blank playlist on Spotify
        print("Response for adding songs to blank playlist on Spotify:", json_response)

# Create an instance of the CreatePlaylist class and call the function to add songs to the new playlist
obj = CreatePlaylist()
